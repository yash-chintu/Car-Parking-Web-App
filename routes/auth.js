const express = require("express");
const bcrypt = require('bcrypt');
const router = express.Router();
const User = require("../models/user");
const Admin = require("../models/admin");

router.route("/login").get((req, res) => {
    if (req.session.user_id) {
    return res.redirect("/user/home/" + req.session.user_id);
    }
    if (req.session.admin_id) {
    return res.redirect("/admin/home/" + req.session.admin_id);
    }
    res.render ("auth/login");
});


router.post("/login", async (req, res) => {
    const { mail, password } = req.body;
    const foundUser = await User.findOne({ mail });
    const foundAdmin = await Admin.findOne({ mail });
    var validUser = false, validAdmin = false;
    if (foundUser) {
        validUser = await bcrypt.compare(password, foundUser.password);
    }
    if (foundAdmin) {
        validAdmin = await bcrypt.compare(password, foundAdmin.password);
    }
    if (validUser) {
        req.session.user_id = foundUser._id;
        res.redirect("/user/home/"+foundUser._id);
    }
    else if (validAdmin) {
        req.session.admin_id = foundAdmin._id;
        res.redirect("/admin/home/");
    }
    else {
        res.redirect("/auth/login");
    }
});


router.get("/register", async (req, res) => {
    res.render("auth/register");
});

router.post("/register", async (req, res) => {
    const { name,mail,pass } = req.body; 
    const password = await bcrypt.hash(pass, 12);
    const user = new User({name, mail, password });
    await user.save();
    res.redirect(("/auth/login"));
});


router.post("/logout", (req, res) => {
    req.session.user_id = null;
    req.session.admin_id = null;
    res.redirect(("/auth/login"));
})


module.exports = router;